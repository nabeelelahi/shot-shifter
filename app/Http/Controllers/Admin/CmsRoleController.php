...
class CmsRoleController extends Controller
{
    ...
    public function ajaxListing(Request $request)
    {
        ...
        $dataTableRecord = CmsRole::dataTableRecords($fields);
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name'           => 'required|min:3|max:50|unique:cms_roles,name,NULL,deleted_at',
            'is_super_admin' => 'required|in:0,1',
        ]);
        if ($validator->fails()) {
            return redirect()->back()->withErrors($validator)->withInput();
        }
        $record = CmsRole::createRole($request->all());
    ...
    {
        $this->_data['record']     = CmsRole::getRecordBySlug($slug);
    ...
    public function update(Request $request,$slug)
    {
        $validator = Validator::make($request->all(), [
            'name'           => [
                ...
                'min:3',
                'max:50',
                Rule::unique('cms_roles')->whereNull('deleted_at')->ignore($slug, 'slug')
            ],
            'is_super_admin' => 'required|in:0,1',
        ]);
        if ($validator->fails()) {
        ...
        $record = CmsRole::updateRole($request->all(),$slug);
        return redirect()->route('cms-roles-management.edit',['cms_roles_management' => $record->slug])
    ...
    {
        CmsRole::deleteRecord($request['id']);
        $message = is_array($request['id']) ? 'Selected records have been deleted successfully.' : 'Record has been deleted successfully';

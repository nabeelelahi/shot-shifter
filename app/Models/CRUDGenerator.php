...
trait CRUDGenerator
{
    ...
    {
        //var_dump($this->__is_cache_record); exit;
        $this->_request = $request;
        ...
        if( Cache::has($this->__cache_signature) && $this->__is_cache_record === true ){
            ...
            if( $this->__is_cache_record === true )
                Cache::tags([$this->table])->put($this->__cache_signature,$data,now()->addDays($this->__cache_expire_time));
        }
        return $data;
    }
    ...
    {
        $this->_request = $request;
        if( Cache::has($this->table .'_' . $slug ) && $this->__is_cache_record === true ){
            ...
            //store record in cache
            if( $this->__is_cache_record === true )
                Cache::tags([$this->table])->put($this->table .'_' . $slug,$data,now()->addDays($this->__cache_expire_time));
        }
        return $data;
    }
    ...
    public function deleteRecord($request, $slug)
    {
        ...
        if( count($records) )
            foreach($records as $record){
                Cache::forget( $this->table .'_' . $record->slug );
            }
        //delete record
